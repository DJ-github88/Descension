...
export function validateCharacterSelection(raceId, subraceId, classData, backgroundData) {
  ...
  if (classData && classData.restrictions) {
    ...
    if (allowedSubraces && allowedSubraces.length > 0) {
      if (!allowedSubraces.includes(subraceId)) {
        if (narrativeUnlock) {
          warnings.push(`Narrative Unlock: The ${classData.name} class is restricted to: ${allowedSubraces.join(', ')}. Choosing this requires DM backstory approval. Justification: ${justification}`);
        } else {
  ...
  if (backgroundData && backgroundData.restrictions) {
    ...
    if (allowedSubraces && allowedSubraces.length > 0 && subraceId && !allowedSubraces.includes(subraceId)) {
      if (narrativeUnlock) {
        warnings.push(`Narrative Unlock: The ${backgroundData.name} background is restricted to: ${allowedSubraces.join(', ')}. Choosing this requires DM backstory approval. Justification: ${justification}`);
      } else {
