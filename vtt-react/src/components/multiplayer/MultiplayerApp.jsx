...
const MultiplayerApp = ({ onReturnToSinglePlayer }) => {
  ...
  useEffect(() => {
    ...
    socket.on('token_updated', (data) => {
      ...
      if (token) {
        ...
        if (wasGrantedControl) {
          console.log('👑 [Control] You have been granted control of this token!');
    ...
    socket.on('token_control_granted', (data) => {
      ...
      console.log('🎯 [token_control_granted] received – checking if I am the target:', {
    ...
    // Player → server → GM: receive accept/reject response and show notification
      ...
      if (isGMRef.current) {
        ...
        if (addCombatNotification) {
          addCombatNotification({
            type: 'system',
            content: accepted
              ? `✅ ${playerName} accepted command of the creature`
              : `❌ ${playerName} declined command of the creature`,
            timestamp: new Date().toISOString()
          });
        }
      ...
      {pendingControlOffer && (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ position: 'absolute', top: '10px', left: '14px', color: '#c9a070', opacity: 0.6, fontSize: '16px' }}>✦</div>
            <div style={{ position: 'absolute', top: '10px', right: '14px', color: '#c9a070', opacity: 0.6, fontSize: '16px' }}>✦</div>

            <div style={{ fontSize: '44px', marginBottom: '14px', filter: 'drop-shadow(0 2px 4px rgba(0,0,0,0.3))' }}>🗡️</div>
