    ...
    Object.entries(wallData).forEach(([wallKey, wallData_item]) => {
      ...
      if (isInteractive) {
        ...
        if (pattern && entry.img && !isGreyedOut) {
          ...
          // We also ensure it's smooth by using the world-relative phase
    ...
    if (windowOverlays && Object.keys(windowOverlays).length > 0) {
      Object.entries(windowOverlays).forEach(([key, windowData]) => {
        ...
        if (shouldFilterByFOV) {
          ...
          const isWindowVisible = visibleAreaSet && tileKeys.some(k => visibleAreaSet.has(k));
          const isWindowExplored = getExploredArea &&
            (getExploredArea(wgx, wgy) || getExploredArea(wgx + 1, wgy) ||
              getExploredArea(wgx, wgy + 1) || getExploredArea(wgx + 1, wgy + 1));

          if (!isWindowVisible && !isWindowExplored) return; // Hidden by unexplored fog
          isWindowGreyedOut = !isWindowVisible; // Dim for explored-but-not-visible
        }
        ...
        ctx.save();
        if (isWindowGreyedOut) ctx.globalAlpha = 0.4;
