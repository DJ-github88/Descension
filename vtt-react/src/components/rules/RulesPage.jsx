...
const CLASS_DATA_LOADERS = {
  ...
  'Inquisitor': () => import('../../data/classes/inquisitorData').then(m => m.INQUISITOR_DATA),



  'Harbinger': () => import('../../data/classes/harbingerData').then(m => m.HARBINGER_DATA),



  'Gambit': () => import('../../data/classes/gambitData').then(m => m.GAMBIT_DATA),



  // 'Deathcaller' and 'Lichborne' merged into Revenant as Phase 1.10 consolidation

  'Revenant': () => import('../../data/classes/revenantData').then(m => m.REVENANT_DATA),
  ...
  'Animist': () => import('../../data/classes/animistData').then(m => m.ANIMIST_DATA),


  'Arcanoneer': () => import('../../data/classes/arcanoneerData').then(m => m.ARCANONEER_DATA),


  'Shaper': () => import('../../data/classes/shaperData').then(m => m.SHAPER_DATA),
  ...
  'Apex': () => import('../../data/classes/apexData').then(m => m.APEX_DATA),
  ...
  // 'Oracle' removed (absorbed into Augur as Seer specialization)
...
const CLASS_ICON_MAP = {
  ...
  // 'Oracle' icon removed (absorbed into Augur)
...
const processMarkdownContent = (text) => {
  ...
  const processedParagraphs = paragraphs.map(pText => {
    let p = pText.trim();
    if (!p) return '';

    // Handle list lines
    if (p.startsWith('- ') || p.startsWith('• ')) {
      // Split by newline and process each list item
      const items = p.split('\n').map(item => {
        let cleanItem = item.replace(/^[\u2022\-]\s*/, '').trim();
        // Process bold, italic, links inside list item
    ...
    // Check if this paragraph is a standalone header (starts and ends with ** and no other ** inside)
    const isStandaloneHeader = p.startsWith('**') && p.endsWith('**') && !p.slice(2, -2).includes('**');
    ...
    if (isStandaloneHeader) {
      return `<p class="rules-standalone-header">${p}</p>`;
    }
    return `<p>${p}</p>`;
  });
...
const processMarkdown = (text) => {
  ...
  // Split lines and group into blockquote and standard text blocks
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (currentBlock === null) {
      currentBlock = {
        type: isQuote ? 'blockquote' : 'text',
        lines: [isQuote ? line.replace(/^>\s?/, '') : line]
      };
    } else if (currentBlock.type === 'blockquote' && isQuote) {
      currentBlock.lines.push(line.replace(/^>\s?/, ''));
    } else if (currentBlock.type === 'text' && !isQuote) {
  ...
  const processedBlocks = blocks.map(block => {
    ...
    if (block.type === 'blockquote') {
      return `<blockquote>${processMarkdownContent(content)}</blockquote>`;
    } else {
  ...
  result = autoLinkTerminology(result);
...
const RulesPage = () => {
  ...
  const handleSubcategoryClick = (categoryId, subcategoryId, sectionIndex = null, tabId = null) => {
    // Map legacy or shorthand subcategory IDs to correct ones
    let targetSubcategory = subcategoryId;
    if (categoryId === 'world-lore' && subcategoryId === 'regions') {
