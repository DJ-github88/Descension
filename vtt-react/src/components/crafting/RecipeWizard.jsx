...
function RecipeWizard({ isOpen, onClose, onSave, onWindowPositionChange, onRecipeDataChange, initialData }) {
    ...
    const saveRecipe = () => {
        ...
        if (addAvailableRecipe && typeof addAvailableRecipe === 'function') {
            ...
            // Fallback: access directly from store if function is not available
            const store = useCraftingStore.getState();
            if (store.addAvailableRecipe) {
                store.addAvailableRecipe(recipe);
            } else {
                console.error('addAvailableRecipe is not available in crafting store');
                return;
            }
        }
                        ...
                        <div className="recipe-review">
                            ...
                            <div className="review-section">
                                ...
                                <div className="result-item">
                                    <img 
                                        src={getIconUrl(resultItem?.iconId || 'inv_misc_questionmark', 'items')}
                                        alt={resultItem?.name}
                                        onError={(e) => {
                                            e.target.src = getIconUrl('inv_misc_questionmark', 'items');
                                        }}
