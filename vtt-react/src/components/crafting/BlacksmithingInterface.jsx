...
function BlacksmithingInterface({ onBack, activeTab, onTabChange }) {
    ...
    const knownRecipes = (typeof getKnownRecipesForProfession === 'function')
        ...
        : (() => {
            const knownIds = storeKnownRecipes?.['blacksmithing'] || [];
    ...
    useEffect(() => {
        ...
        const queuedItems = craftingQueue.filter(item => item.status === 'queued' && item.recipe?.profession === 'blacksmithing');
    ...
    const completeCrafting = (craftingItem) => {
        ...
        if (typeof gainExperience === 'function') {
            experienceResult = gainExperience('blacksmithing', xpGained);
        } else {
            // Fallback: manually calculate experience and level up
            const store = useCraftingStore.getState();
            const currentLevel = store.professionLevels?.['blacksmithing'] ?? SKILL_LEVELS.UNTRAINED.level;
            const currentExperience = store.professionExperience?.['blacksmithing'] ?? 0;
            ...
            if (currentLevel >= 9) {
                ...
                useCraftingStore.setState(state => ({
                    professionExperience: {
                        ...
                        'blacksmithing': newExperience
                    },
                    professionLevels: {
                        ...state.professionLevels,
                        'blacksmithing': newLevel
                    }
                }));
        ...
        if (experienceResult && experienceResult.leveledUp) {
            addLootNotification({
                type: 'skill_increase',
                message: `Blacksmithing skill increased to ${Object.values(SKILL_LEVELS).find(s => s.level === experienceResult.newLevel)?.name}!`,
                timestamp: Date.now()
            });
        } else {
            ...
            addLootNotification({
                ...
                message: `Gained ${xpGained} Blacksmithing experience.`,
                timestamp: Date.now()
            });
        }
        ...
        <div className="recipes-content">
            ...
            {knownRecipes.length === 0 ? (
                <div className="no-recipes">
                    ...
                    <p>You haven't learned any Blacksmithing recipes yet.</p>
                        ...
                        <div className="crafting-info">
                            ...
                            {selectedRecipe.experienceGained && (
                                <div className="crafting-xp">
                                    ...
                                    <span className="xp-value">+{selectedRecipe.experienceGained} Blacksmithing</span>
                                </div>
                            )}
                        </div>
...
export default BlacksmithingInterface;
