...
function creatureLibraryReducer(state, action) {
  switch (action.type) {
    case LIBRARY_ACTION_TYPES.ADD_CREATURE: {
      ...
      console.log('📝 Library: Adding creature with ID:', newCreature.id, 'Name:', newCreature.name);
    ...
    case LIBRARY_ACTION_TYPES.SORT_CREATURES: {
      return {
        ...state,
...
// Provider component
export function CreatureLibraryProvider({ children }) {
  const creatureStore = useCreatureStore();

  // Initialize state with initialState first
    ...
    const loadFromFirebase = async () => {
      try {
        ...
        if (user?.uid && !user?.isGuest) {
          const { loadUserCreatures } = await import('../../../services/firebase/userCreaturesService');
          const firebaseCreatures = await loadUserCreatures(user.uid);

          if (!cancelled && firebaseCreatures && firebaseCreatures.length > 0) {
            console.log(`📚 Loaded ${firebaseCreatures.length} creatures from Firebase for user ${user.uid}`);
            // Merge with any existing local creatures (prefer Firebase versions)
            const existingIds = new Set((creatureStore.creatures || []).map(c => c.id));
        ...
        console.warn('⚠️ Could not load creatures from Firebase, using local data:', err);
      }
    };
...
export const useCreatureLibraryDispatch = () => {
...
export const libraryActionCreators = {
  ...
  sortCreatures: (sortOrder) => ({
    type: LIBRARY_ACTION_TYPES.SORT_CREATURES,
    payload: sortOrder
  }),
