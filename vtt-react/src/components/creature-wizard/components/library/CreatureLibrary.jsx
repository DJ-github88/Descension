...
const CreatureLibrary = ({ onEdit, onCreateNew }) => {
  ...
  const confirmDeleteCreature = async () => {
    if (creatureToDelete) {
      ...
      // Persist deletion to Firebase (user is already in scope from useAuthStore hook)
      try {
        if (user?.uid && !user?.isGuest) {
          const { deleteUserCreature } = await import('../../../../services/firebase/userCreaturesService');
          await deleteUserCreature(user.uid, creatureToDelete.id);
        }
      } catch (err) {
