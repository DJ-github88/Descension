...
// Thematic resistance descriptions matching spell wizard format
              ...
              <div style={{ marginBottom: '2px' }}>
                ...
                <span style={{ fontSize: '9px', color: '#388e3c' }}>
                  {Object.entries(creature.resistances).slice(0, 2).map(([type, value]) => {
                    const thematicDesc = getThematicResistanceDescription('resistant', type);
                    return thematicDesc.split('(')[0].trim(); // Just the name part
                  }).join(', ')}
                </span>
              </div>
            )}
              ...
              <div>
                ...
                <span style={{ fontSize: '9px', color: '#f44336' }}>
                  {Object.entries(creature.vulnerabilities).slice(0, 2).map(([type, value]) => {
                    const thematicDesc = getThematicResistanceDescription('vulnerable', type);
                    return thematicDesc.split('(')[0].trim(); // Just the name part
                  }).join(', ')}
                </span>
              </div>
