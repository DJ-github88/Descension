...
const Step5Resources = () => {
 ...
 const classResourceCategories = [
  ...
  },
  {
   ...
   color: '#8A2BE2',
   subcategories: [
    { id: 'mayhem_generate', name: 'Mayhem Modifiers Generate', shortName: '+MM', icon: faRandom, color: '#8A2BE2' },
    { id: 'mayhem_spend', name: 'Mayhem Modifiers Spend', shortName: '-MM', icon: faRandom, color: '#8A2BE2' }
   ]
  },
  {
   id: 'fate_mechanics',
   name: 'Fate Mechanics',
   description: 'Fate Weaver token mechanics',
   icon: faRandom,
   color: '#2E86C1',
   subcategories: [
    { id: 'threads_generate', name: 'Threads of Destiny Generate', shortName: '+Threads', icon: faRandom, color: '#9370DB' },
    { id: 'threads_spend', name: 'Threads of Destiny Spend', shortName: '-Threads', icon: faRandom, color: '#9370DB' }
   ]
  },
  {
   id: 'inferno_veil',
  ...
  },
  {
   ...
   color: '#4169E1',
   subcategories: [
    { id: 'time_shard_generate', name: 'Time Shards Generate', shortName: 'Generate', icon: faClock, color: '#4169E1' },
    { id: 'time_shard_cost', name: 'Time Shards Cost', shortName: 'Cost', icon: faHourglass, color: '#4169E1' },
 ...
 const getResourceHintText = (resourceType, level) => {
  switch (resourceType) {
   ...
   case 'inferno_ascend':
    return 'How many Inferno levels to gain when casting (usually 1-3)';
   case 'inferno_descend':
    return 'How many Inferno levels to lose when casting (usually 1-3)';
   case 'arcane_sphere':
   case 'holy_sphere':
   case 'shadow_sphere':
   case 'fire_sphere':
   ...
   case 'healing_sphere':
   case 'chaos_sphere':
    return 'Number of spheres consumed (usually 1-3)';
   case 'time_shard_generate':
    return 'Number of Time Shards generated by basic spells (usually 1-2)';
   case 'time_shard_cost':
    return 'Number of Time Shards consumed for Temporal Flux abilities (usually 2-6)';
   case 'temporal_strain_gain':
    return 'Amount of Temporal Strain gained when casting Flux abilities (usually 1-5)';
   case 'temporal_strain_reduce':
    return 'Amount of Temporal Strain reduced by cleansing spells (usually 1-3)';
   case 'devotion_required':
    return 'Minimum Devotion Level required to cast (0-6)';
   case 'devotion_cost':
    return 'Number of Devotion Levels consumed for amplified spells (usually 1-5)';
   case 'devotion_gain':
    return 'Number of Devotion Levels gained (through self-damage or sacrifice effects)';
   case 'mayhem_generate':
    return 'Mayhem Modifiers generated (use fixed value or dice formula like 2d4)';
   case 'mayhem_spend':
    return 'Mayhem Modifiers consumed when casting (fixed value)';
   case 'threads_generate':
    return 'Threads generated from failures/negative outcomes (e.g., 1 or 2, or formula)';
   case 'threads_spend':
    return 'Threads spent to manipulate cards (usually 1-8 depending on spell)';
   case 'bloodTokens':
           ...
           <div className="resource-subcategory-panel">
            {category.subcategories.map(sub => (
             <div
              ...
              title={sub.name}
             >
              <div className={`resource-type-icon small ${sub.id === 'note_ii' ? 'smaller-icon' : ''}`} style={{ color: sub.color }}>
