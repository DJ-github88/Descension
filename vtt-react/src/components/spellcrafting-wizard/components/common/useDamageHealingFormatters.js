...
const useDamageHealingFormatters = ({ spell, variant, enhanceFormulaDisplay }) => {
  ...
  if (critConfig.critOnlyEffect) {
   ...
   if (effects.length > 0) {
    const effectNames = effects.map(effect => {
     // Format effect names nicely - convert marks to spaces and capitalize each word
 ...
 const getDamageTypes = () => {
  ...
  if (damageTypesSet.size === 0 && hasDamageConfig) {
   ...
   if (spellText.includes('sacred') || spellText.includes('sacred') || spellText.includes('sacred light')) damageTypesSet.add('sacred');
