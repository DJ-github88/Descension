 ...
 * Channeling System
 * 
 * Provides a complete implementation for spells that are maintained over time,
...
export function getMaxChannelDuration(spellLevel, casterState) {
 ...
 if (casterState.attributes) {
  ...
  // Bonus from spirit (for sacred casters)
  if (casterState.castingType === 'sacred' && casterState.attributes.spirit) {
...
export function getChannelTickTiming(channelConfig, gameState) {
 ...
 switch (tickFrequency) {
  ...
  case 'per_turn':
  // Ticks once per turn ( during action phase)
