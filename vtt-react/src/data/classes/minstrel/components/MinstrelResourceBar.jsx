...
const MinstrelResourceBar = ({
            ...
            return ReactDOM.createPortal(
                <div ref={tooltipRef} className="unified-resourcebar-tooltip pathfinder-tooltip minstrel-tooltip" style={{ position: 'fixed', left: 0, top: 0, opacity: 0, pointerEvents: 'none', zIndex: 100000 }}>
                    <ClassTip
                        ...
                        mechanic={`${note.function} — ${note.description} Generated by ${note.generatedBy}; spent when a cadence resolves. Stacks to ${maxPerNote}.`}
                        status={[
                            count > 0
                                ? `${count} banked — spendable in cadences.`
                                : 'Empty — play builder spells to bank this note.',
                            readyForms.length > 0
                                ? `Ready: ${readyForms.map(f => f.name).join(', ')}.`
                                : forms.length > 0
                                    ? `No cadence ready — needs partner notes (${forms.length} cadences use this note).`
                                    : null,
                        ]}
                        usage={canEdit ? 'Click note to open its tuner (party view: click cycles) · Right-click -1 · CADENCE opens the songbook.' : null}
                    />
                </div>,
                document.body
            );
        ...
        return ReactDOM.createPortal(
            <div ref={tooltipRef} className="unified-resourcebar-tooltip pathfinder-tooltip minstrel-tooltip" style={{ position: 'fixed', left: 0, top: 0, opacity: 0, pointerEvents: 'none', zIndex: 100000 }}>
                <ClassTip
                    ...
                    stateTone={readyCadences.length > 0 ? 'good' : 'neutral'}
                    mechanic="Bank notes by casting builder spells (max 5 per pitch, I-VII); resolve cadences to spend them. Notes decay 1 per minute out of combat."
                    status={[
                        `Held: ${totalBanked}/35 notes.`,
                        readyCadences.length > 0
                            ? `Ready cadences: ${readyCadences.map(c => c.name).join(', ')}.`
                            : 'No complete cadence — bank the missing pitches.',
                        'Open CADENCE for the full songbook.'
                    ]}
                    usage="Click a note to tune it · Right-click -1 · CADENCE / Show Available Cadences opens the songbook"
    ...
    const renderEditMenu = () => {
        ...
        return ReactDOM.createPortal(
            <div
                ref={editMenuRef}
                className="unified-context-menu compact context-menu-container minstrel-menu-container note-menu class-resource-menu"
                ...
                onMouseEnter={(e) => { e.stopPropagation(); setShowStaffTooltip(false); }}
                ...
                <div className="context-menu-main">
                    <div className="context-menu-section">
                        <div className="context-menu-section-header" style={{ color: note.color }}>
                            ...
                            <span style={{ fontSize: '0.75rem', color: 'rgba(254,243,199,0.55)', marginLeft: 'auto' }}>{count}/{maxPerNote}</span>
                        </div>
                        <div style={{ fontSize: '0.78rem', color: 'rgba(254,243,199,0.75)', marginBottom: '6px', lineHeight: 1.35 }}>{note.function}, {note.description}</div>
    ...
    const renderCompactPanel = () => {
        ...
        return ReactDOM.createPortal(
            <div
                ref={panelRef}
                className="minstrel-cadence-modal minstrel-menu-container class-resource-menu"
                onMouseDown={(e) => { e.stopPropagation(); if (e.nativeEvent?.stopImmediatePropagation) e.nativeEvent.stopImmediatePropagation(); }}
                onClick={(e) => { e.stopPropagation(); if (e.nativeEvent?.stopImmediatePropagation) e.nativeEvent.stopImmediatePropagation(); }}
                onMouseEnter={(e) => { e.stopPropagation(); setShowStaffTooltip(false); }}
                onMouseMove={(e) => e.stopPropagation()}
