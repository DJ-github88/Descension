...
const FROSTWOOD = {
  ...
  perchtaweave: [
    ctrl({ id: 'perchtaweave_fog_spinning', name: 'Fog-Spinning', icon: 'spell_shadow_fade', school: 'wyrd', desc: 'Spawns memory-fog in a wide radius, dissolving minor memories.', ap: 2, mana: 15, cd: 2, range: 0, targetingType: 'area', rangeType: 'self_centered', areaShape: 'circle', areaSize: 60, save: ['spirit', 14], duration: 3, controlType: 'zone', effects: [{ id: 'fog_zone', name: 'Memory-Fog', description: 'DC 14 SPI each round or lose one minor memory.' }] }),
    dmg({ id: 'perchtaweave_distaff_strike', name: 'Distaff-Strike', icon: 'spell_shaman_stormstrike', school: 'physical', desc: 'A whistling blow from her rune-carved distaff.', ap: 1, mana: 0, cd: 0, range: 15, rangeType: 'melee', formula: '2d8 + 4', types: ['physical'] }),
    debuff({ id: 'perchtaweave_straw_judgment', name: 'Straw-Judgment', icon: 'spell_shadow_unstableaffliction', school: 'blight', desc: 'Fills a negligent household\'s belly with straw.', ap: 2, mana: 25, cd: 5, range: 30, save: ['constitution', 15], duration: 3, debuffType: 'incapacitated', effects: [{ id: 'straw_belly', name: 'Belly of Straw', description: 'Incapacitated for 24 hours; not lethal but deeply disturbing.' }] }),
  ],
  dullahaze: [
    debuff({ id: 'dullahaze_fear_mirror', name: 'Fear-Mirror', icon: 'spell_shadow_phantasm', school: 'wyrd', desc: 'Its mask reflects the viewer\'s deepest fear.', ap: 1, mana: 10, cd: 1, range: 30, save: ['spirit', 14], duration: 3, debuffType: 'statusEffect', effects: [{ id: 'frightened', name: 'Frightened', description: 'Frightened for 3 rounds.' }] }),
    ctrl({ id: 'dullahaze_spine_whip', name: 'Spine-Whip', icon: 'spell_shadow_soulleech', school: 'physical', desc: 'A barbed spine lashes out, dragging the victim closer.', ap: 2, mana: 10, cd: 1, range: 15, rangeType: 'melee', formula: '2d6 + 4', types: ['physical'], save: ['agility', 14], saveOutcome: 'half_damage', controlType: 'forcedMovement', duration: 0, effects: [{ id: 'pulled', name: 'Pulled', description: 'Pulled 10 ft toward the rider.', config: { distance: 10, movementType: 'pull' } }] }),
    passive({ id: 'dullahaze_fog_mount', name: 'Fog-Mount', icon: 'spell_shadow_summonvoidwalker', school: 'arcane', desc: 'Its steed is woven of fog; immune to physical harm.', effects: [{ id: 'fog_mount', name: 'Fog-Mount', description: 'Mount immune to physical damage; takes double radiant.' }] }),
  ],
  cu_sithbane: [
  ...
  waldwisp: [
    dmg({ id: 'waldwisp_red_cap_spark', name: 'Red-Cap-Spark', icon: 'spell_fire_firebolt', school: 'ember', desc: 'Flicks a tiny ember from its crimson cap.', ap: 1, mana: 0, cd: 0, range: 30, formula: '1d4', types: ['ember'] }),
    ctrl({ id: 'waldwisp_root_rattle', name: 'Root-Rattle', icon: 'spell_nature_stranglevines', school: 'storm', desc: 'A clattering rattle of roots deafens nearby foes.', ap: 1, mana: 5, cd: 1, range: 0, targetingType: 'area', rangeType: 'self_centered', areaShape: 'circle', areaSize: 10, save: ['constitution', 12], duration: 1, controlType: 'incapacitated', effects: [{ id: 'deafened', name: 'Deafened', description: 'Deafened for 1 round.' }] }),
    passive({ id: 'waldwisp_fear_feed', name: 'Fear-Feed', icon: 'spell_shadow_twistedfaith', school: 'wyrd', desc: 'Grows stronger near frightened creatures.', effects: [{ id: 'fear_feed', name: 'Fear-Feed', description: '+2 to all rolls for each frightened creature within 30 ft.' }], range: 30, areaShape: 'circle', areaSize: 30 }),
  ],
};
...
const NORDHALLA = {
  ...
  trollstone: [
    dmg({ id: 'trollstone_roll_attack', name: 'Roll-Attack', icon: 'spell_shaman_stormstrike', school: 'physical', desc: 'An immense boulder-charge that can pin the victim.', ap: 2, mana: 0, cd: 2, range: 10, formula: '2d10 + 4', types: ['physical'], save: ['agility', 15], saveOutcome: 'no_effect', controlType: 'restraint', duration: 1, effects: [{ id: 'pinned', name: 'Pinned', description: 'Pinned beneath the stone.' }] }),
    passive({ id: 'trollstone_stone_regen', name: 'Stone-Regeneration', icon: 'spell_nature_regeneration', school: 'primal', desc: 'Reknits its rocky hide each round.', effects: [{ id: 'regen', name: 'Regeneration', description: 'Heals 10 HP per round; only concentrated runic light truly damages it.' }] }),
    util({ id: 'trollstone_boulder_sleep', name: 'Boulder-Sleep', icon: 'spell_druid_camouflage', school: 'primal', desc: 'Becomes indistinguishable from a natural boulder.', ap: 1, mana: 0, cd: 5, range: 0, rangeType: 'self', target: 'self', targetRestrictions: ['self'], utilityType: 'movement', duration: 1, effects: [{ id: 'inert', name: 'Inert', description: '+10 to stealth; appears as a natural boulder.' }] }),
  ],
  lindwyrm_burrow: [
  ...
  fenrispup: [
    dmg({ id: 'fenrispup_puppy_bite', name: 'Puppy-Bite', icon: 'ability_druid_gore', school: 'physical', desc: 'A surprisingly powerful bite.', ap: 1, mana: 0, cd: 0, range: 5, rangeType: 'melee', formula: '2d6 + 4', types: ['physical'] }),
    passive({ id: 'fenrispup_bound_hunter', name: 'Bound-Hunter', icon: 'ability_hunter_aspectofthehawk', school: 'physical', desc: 'Tracks oath-breakers across a hundred miles.', effects: [{ id: 'track_oathbreaker', name: 'Oath-Breaker Track', description: 'Tracks any creature who broke a sworn vow within 100 miles.' }] }),
    passive({ id: 'fenrispup_gleipnir_collar', name: 'Gleipnir-Collar', icon: 'spell_shadow_unbreakablewill', school: 'arcane', desc: 'A braided collar that contains its true size.', effects: [{ id: 'collar', name: 'Gleipnir-Collar', description: 'Collar prevents growth; if removed, becomes a continent-ending catastrophe.' }] }),
  ],
  disablot_wraith: [
  ...
  kraken_ice: [
    ...
    util({ id: 'kraken_ice_breaker', name: 'Ice-Breaker', icon: 'spell_frost_frostblast', school: 'rime', desc: 'Shatters the ice beneath a vessel.', ap: 3, mana: 20, cd: 5, range: 30, utilityType: 'environment', effects: [{ id: 'shatter', name: 'Ice-Shatter', description: 'Shatters ice beneath a vessel; the ship sinks in 2 rounds.' }] }),
  ],
  marmennill: [
