   ...
   addCharacterToken: (position, playerId = null, targetMapId = null, sendToServer = true, characterSnapshot = null) => {
    ...
    Promise.all([
     ...
     if (gameStore.defaultViewFromToken) {
      ...
      // CRITICAL FIX: Enable viewingFromToken for players when shifting fog is enabled
      // This ensures the afterimage/memory system works for players
      console.log('ðŸ‘ï¸ [Afterimage] Auto-enabling viewingFromToken for player (shifting fog enabled)');
      levelEditorStore.setViewingFromToken({
       id: tokenId,
       type: 'character',
       characterId: playerId || 'local_player',
