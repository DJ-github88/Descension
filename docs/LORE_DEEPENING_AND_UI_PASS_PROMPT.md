...
- **Ecological inevitability**  -  the tradition's core mechanic must be a rational response to the region's specific geography/climate/threat. (Berserker Rage ↔ Sundale's forge-heat + Nordhalla's starvation; Augur entrail-reading ↔ Nordhalla's glacier-preservation of the dead; etc.) Where the link is weak, strengthen it in `overview.originStory` and the class's `loreDictionary` `fullEntry`.
